...
function Home({navigation}) {
    ...
    let topRate = useSelector(state => state.topRate.topRate);
    const topRateLoader = useSelector(state => state.topRate.loader);

    const [search,   setSearch] = useState('');
    const [_topRate, setTopRate] = useState([]);
	...
	useFocusEffect(
		React.useCallback(() => setTopRate(topRate), [])
	);
